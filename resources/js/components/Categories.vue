	...
	<div>
		...
		<div class="input-group mb-3 no-gutters">
			...
			<input type="text" v-model="category" class="form-control col-8 px-2"
				   id="category" max="15" name="category">
		...
		<div class="my-2 row">
			<div v-for="(category, index) in categories" class="mx-2">
                <label :for="'category' + category.id" class="my-1">{{ category.name }}</label>
                <input type="checkbox" :id="'category' + category.id" v-model="category.checked"
                       @click="checkCategory">
			</div>
		</div>
